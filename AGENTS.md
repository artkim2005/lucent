...
You are a **principal-level full-stack engineer and AI implementation agent** working on **LUCENT**, a production-style AI-powered news analysis website.
...
LUCENT collects real news articles from configured sources, analyzes them with AI, stores them in Supabase, and displays reader-friendly sentiment and framing insights.
...
All action routes that start or mutate work must require a shared admin secret sent as the `x-LUCENT-admin-secret` request header. Store the value in the `LUCENT_ADMIN_SECRET` environment variable.
...
- Trigger with `POST /api/scrape` and require the `x-LUCENT-admin-secret` header (section 15).
...
For API features, share the exact curl commands needed to hit each endpoint, including the correct method, headers, and JSON body. Always include the `x-LUCENT-admin-secret` header where required.
...
Use Oxylabs Scheduler to run monthly scraping for active source homepages stored in Supabase.
...
Each call to the sync route that creates a new schedule leaves behind old schedules on Oxylabs if DB rows were deleted and re-created. These orphaned schedules still run monthly and count against the Oxylabs bill.
...
- `POST /api/oxylabs/schedules` â€” tells Oxylabs what to scrape monthly. Done once per source set.
- Vercel Cron config â€” tells Vercel to call `/api/cron/pipeline` at 3:15 AM UTC on the 1st of every month. Done once via `vercel.json`.
...
1. Oxylabs Scheduler runs its jobs at 3:00 AM UTC on the 1st of every month.
...
Do not use `LUCENT_ADMIN_SECRET` to protect the cron route. Do not add `CRON_SECRET` to `.env.local`.
...
- Vercel Cron config â€” registers the automatic monthly trigger
- Cron pipeline route â€” chains scheduled result processing then AI analysis

- **Oxylabs Scheduler** tells Oxylabs to scrape our active source homepages every month and store the results. That’s set up once with a route in our app.
...
The request must include the `x-LUCENT-admin-secret` header.
...
| `LUCENT_ADMIN_SECRET`                                                         | Shared secret for `x-LUCENT-admin-secret` on action routes (section 15)                 | server only     |
